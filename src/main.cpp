...
#if defined(NDEBUG)
# error "IBTMaxcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7f10d123ef8fe138c923f7042dc192643851d1a7001778b9c041c13c9c44ecc0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // IBTMaxcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "IBTMaxcoin Signed Message:\n";
...
{
    // IBTMaxcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // nSubsidy >>= (nHeight / 840000); // IBTMaxcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10000 * 24 * 60 * 60; // IBTMaxcoin: never
static const int64 nTargetSpacing = 1 * 30; // IBTMaxcoin: 30secs
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // IBTMaxcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // IBTMaxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7f10d123ef8fe138c923f7042dc192643851d1a7001778b9c041c13c9c44ecc0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2017-11-09 Xi's up, Trump is down, but it may not matter | CNN NEWS";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1510191204;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 147810;

        if (fTestNet)
        {
            block.nTime    = 1510191205;
            block.nNonce   = 147810;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa9ee6baa942dba355c5694768c31daa32846e74706e741d157b67ec97397c3a1"));
...
unsigned char pchMessageStart[4] = { 0xd6, 0xf3, 0xb7, 0xc5 }; // IBTMaxcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IBTMaxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IBTMaxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IBTMaxcoinMiner(CWallet *pwallet)
{
    printf("IBTMaxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ibtmaxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running IBTMaxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IBTMaxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IBTMaxcoinMiner, pwallet));
}
